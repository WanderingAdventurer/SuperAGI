...
from superagi.models.db import connect_db  # uses env to build the engine
...
# -----------------------------------------------------------------------------
# Env / DB URL (for DBSessionMiddleware)
# -----------------------------------------------------------------------------
db_host = sa_get_config('DB_HOST')
db_url = sa_get_config('DB_URL')
db_username = sa_get_config('DB_USERNAME')
db_password = sa_get_config('DB_PASSWORD')
db_name = sa_get_config('DB_NAME')
env = sa_get_config('ENV', "DEV")

# Build DB URL string for DBSessionMiddleware only (engine comes from connect_db)
if db_url is None:
    if db_username is None:
        db_url_mw = f'postgresql://{db_host}/{db_name}'
    else:
        db_url_mw = f'postgresql://{db_username}:{db_password}@{db_host}/{db_name}'
else:
    parsed = urlparse(db_url)
    db_url_mw = f"{parsed.scheme}://{parsed.netloc}{parsed.path}"

# Global engine + sessionmaker for startup tasks
engine = connect_db()
SessionLocal = sessionmaker(bind=engine)

# FastAPI-SQLAlchemy middleware (used by controllers that call `db.session`)
app.add_middleware(DBSessionMiddleware, db_url=db_url_mw)
...
# Auth / JWT config
# -----------------------------------------------------------------------------
JWT_SECRET = os.getenv("JWT_SECRET_KEY") or sa_get_config("JWT_SECRET_KEY")
if not JWT_SECRET:
    logger.error("❌ JWT_SECRET_KEY missing. /login will fail until you set it.")

class Settings(BaseModel):
    authjwt_secret_key: str = JWT_SECRET or "unset"  # FastAPI-JWT-Auth requires *something*
...
def create_access_token(email: str, Authorize: AuthJWT) -> str:
    expiry_hours = sa_get_config("JWT_EXPIRY", 200)
    try:
...
def replace_old_iteration_workflows(session):
    ...
    for t in templates:
        iw = IterationWorkflow.find_by_id(session, t.agent_workflow_id)
        if not iw:
            continue
        if iw.name in name_map:
            awf = AgentWorkflow.find_by_name(session, name_map[iw.name])
            if awf:
                t.agent_workflow_id = awf.id
                session.commit()
...
async def startup_event():
    ...
    try:
        # Only do DB work if tables exist
        if table_exists("agent_workflows"):
            ...
            obsolete = session.query(AgentWorkflow).filter(AgentWorkflow.name.not_in(allowed)).all()
            for ow in obsolete:
                session.delete(ow)
        ...
        if table_exists("users") and table_exists("organisations"):
            ...
            if default_user:
                org = session.query(Organisation).filter_by(id=default_user.organisation_id).first()
                if org:
        ...
        if env != "PROD":
            ...
            if table_exists("organisations"):
                market_org_id = sa_get_config("MARKETPLACE_ORGANISATION_ID")
                if market_org_id:
                    m_org = session.query(Organisation).filter_by(id=market_org_id).first()
                    if m_org:
                        register_marketplace_toolkits(session, m_org)
...
def login(request: LoginRequest, Authorize: AuthJWT = Depends()):
    logger.info("🔐 /login called for email='%s'", request.email)

    if not JWT_SECRET or JWT_SECRET == "unset":
        # fail loudly if server isn't configured
